...
// Проверяет: ArgsParser возвращает NoArgs при отсутствии аргументов
...
// Проверяет: один filter с params сохраняется
...
// Проверяет: несколько filters сохраняются по порядку
...
// Проверяет: filter без params сохраняется
...
// Проверяет: отрицательное число считается param, а не flag
TEST_CASE("ArgsParser: negative number is a param not a flag", "[args_parser]") {
    // -0.5 не должен восприниматься как flag
...
// Проверяет: silence с 3 params сохраняется
...
// Проверяет: generator sin с params сохраняется
...
// Проверяет: полный пример из TZ парсится корректно
...
// Проверяет: filter params сохраняют trailing text для producer validation
