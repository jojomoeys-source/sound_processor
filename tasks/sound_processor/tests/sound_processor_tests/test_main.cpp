...
// Проверяет: Application без аргументов выводит help и возвращает success
...
// Проверяет: Application для bad arguments возвращает error
TEST_CASE("Application: bad arguments return error", "[application]") {
    CHECK(run_application({"sound_processor", "-f", "-i"}) == 1);
}

// Проверяет: Application с generator создаёт output без input
...
// Проверяет: Application без filters копирует input в output
