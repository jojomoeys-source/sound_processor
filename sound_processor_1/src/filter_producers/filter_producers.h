...
inline void require_params_range(const FilterDescriptor& fd, size_t min_n, size_t max_n) {
...
// -f highpass <window_size>
inline IFilter* make_highpass(const FilterDescriptor& fd) {
    detail::require_params(fd, 1);
    const int window = detail::parse_int(fd.params[0], "highpass", "window_size");
    return new HighpassFilter(window);
}

// -f bandpass <low_window_size> <high_window_size>
inline IFilter* make_bandpass(const FilterDescriptor& fd) {
...
// -f reject <low_window_size> <high_window_size>
inline IFilter* make_reject(const FilterDescriptor& fd) {
    detail::require_params(fd, 2);
