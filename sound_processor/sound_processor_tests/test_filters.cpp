...
TEST_CASE("GainFilter: same behaviour as AmplFilter", "[gain]") {
    Waveform w1 = make_waveform({1000, -2000, 0});
    Waveform w2 = make_waveform({1000, -2000, 0});
    AmplFilter ampl(1.5);
    GainFilter gain(1.5);
    ampl.apply(w1);
    gain.apply(w2);
    for (size_t i = 0; i < w1.get_sample_count(); ++i)
        CHECK(w1.get_sample_at(i) == w2.get_sample_at(i));
}

TEST_CASE("GainFilter: negative factor throws", "[gain]") {
    CHECK_THROWS_AS(GainFilter(-0.1), std::invalid_argument);
}
...
TEST_CASE("NormalizeFilter: scales to custom peak", "[normalize]") {
    ...
    f.apply(w);
    // 32767 * 0.5 = 16383 (с округлением)
    CHECK(w.get_sample_at(0) == 16383);
}
...
TEST_CASE("SilenceFilter: inserts silence (shifts tail right)", "[silence]") {
    ...
    // Длина увеличилась на 4410 сэмплов
    CHECK(w.get_sample_count() == 44100 + 4410);
    // Первые 4410 — нули
    CHECK(w.get_sample_at(0)    == 0);
    CHECK(w.get_sample_at(4409) == 0);
    // После вставки — исходные данные
    CHECK(w.get_sample_at(4410) == 500);
}
...
TEST_CASE("SilenceFilter: ms unit", "[silence]") {
    ...
    SilenceFilter f(SilenceFilter::Unit::Ms, 0.0, 100.0);  // 100 ms = 4410 samples
    f.apply(w);
    CHECK(w.get_sample_count() == 44100 + 4410);
    CHECK(w.get_sample_at(0) == 0);
    CHECK(w.get_sample_at(4410) == 100);
}
...
TEST_CASE("TimestretchFilter: factor 2 doubles length", "[timestretch]") {
    Waveform w = make_waveform({0, 1000});
    TimestretchFilter f(2.0);
    f.apply(w);
    CHECK(w.get_sample_count() == 4);
    // Первый сэмпл = 0, последний ~ 1000
    CHECK(w.get_sample_at(0) == 0);
    CHECK(w.get_sample_at(3) == 1000);
}
